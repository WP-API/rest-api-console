...
var authPanel = new ui.AuthPanel('#auth', {
    ...
    referencePanel = new ui.ReferencePanel({
      'search_url' : rest_api_console.search_url,
      'defaultValue' : [],
    ...
    methodSelector = new ui.OptionSelector($('#method'), { values: ['GET', 'POST', 'PUT', 'DELETE', 'OPTIONS'] } ),
    ...
    pathField = new ui.PathField($('#path'), {
      ...
      'itemTemplate': function(item) {
        ...
        var listItem = $("<li></li>"),
            ...
            group = $("<strong></strong>").text(item.namespace).appendTo(listItem.append(" ")),
...
methodSelector.on('change', function() {
  if ( -1 === ['GET', 'DELETE', 'OPTIONS'].indexOf( methodSelector.getValue() ) ) {
    bodyBuilder.enable();
  } else {
    bodyBuilder.disable();
  }
